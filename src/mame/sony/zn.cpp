...
//#define VERBOSE       (LOG_GENERAL | LOG_BANK | LOG_DMA | LOG_MCU | LOG_SEC)
...
#define LOGSEC(...)     LOGMASKED(LOG_SEC, __VA_ARGS__)

zn_state::zn_state(const machine_config &mconfig, device_type type, const char *tag) :
	driver_device(mconfig, type, tag),
	m_maincpu(*this, "maincpu"),
	m_irq(*this, "maincpu:irq"),
...
void zn_state::zn_base(machine_config &config)
{
	...
	m_znmcu->analog<0>().set_ioport("ANALOG1");
	m_znmcu->analog<1>().set_ioport("ANALOG2");
	m_znmcu->dsw().set_ioport("DSW");
	m_znmcu->trackball<0>().set_ioport("TRACKBALL1");
	m_znmcu->trackball<1>().set_ioport("TRACKBALL2");
}
...
void zn_state::driver_start()
{
	...
	save_item(NAME(m_znsecsel));
	save_item(NAME(m_zn2_spu_hack));
}

void zn_state::maincpu_program_map(address_map &map)
{
	...
	map(0x1fa20000, 0x1fa20000).rw(FUNC(zn_state::coin_r), FUNC(zn_state::coin_w));

	// TODO: is this interrupt related?
	map(0x1fa30000, 0x1fa30003).noprw();
	map(0x1fa40000, 0x1fa40003).nopr();

	map(0x1faf0000, 0x1faf07ff).rw(m_at28c16, FUNC(at28c16_device::read), FUNC(at28c16_device::write));
	map(0x1fb20000, 0x1fb20007).r(FUNC(zn_state::unknown_r));
}

void zn_state::zn2_maincpu_program_map(address_map &map)
{
	maincpu_program_map(map);

	// TODO: is this a work around for mismatched CPU & SPU clock?
	map(0x1fa51c00, 0x1fa51dff).nopr();
	map(0x1fa60000, 0x1fa60001).r(FUNC(zn_state::zn2_spu_hack_r));
}
...
void zn_state::coin_w(uint8_t data)
{
	...
	machine().bookkeeping().coin_lockout_w(0, !BIT(m_coin, 1));
	machine().bookkeeping().coin_counter_w(1, BIT(m_coin, 4));
	machine().bookkeeping().coin_lockout_w(1, !BIT(m_coin, 5));
}

uint16_t zn_state::zn2_spu_hack_r()
{
	if (!machine().side_effects_disabled())
	{
		m_zn2_spu_hack ^= 8;

		LOG("%s zn2_spu_hack_r()\n", machine().describe_context(), m_zn2_spu_hack);
	}

	return m_zn2_spu_hack;
}

uint16_t zn_state::unknown_r(offs_t offset, uint16_t mem_mask)
{
	if (!machine().side_effects_disabled())
		LOG("%s unknown_r(0x%04x, 0x%04x)\n", machine().describe_context(), offset, mem_mask);

	return 0xffff;
}

uint8_t zn_state::znsecsel_r()
{
	if (!machine().side_effects_disabled())
		LOGSEC("%s znsecsel_r() 0x%02x\n", machine().describe_context(), m_znsecsel);

	return m_znsecsel;
}

void zn_state::znsecsel_w(uint8_t data)
{
	LOGSEC("%s znsecsel_w(0x%02x)\n", machine().describe_context(), data);

	if (m_cat702[0]) m_cat702[0]->write_select(BIT(data, 2));
	if (m_cat702[1]) m_cat702[1]->write_select(BIT(data, 3));
	m_znmcu->analog_read(BIT(data, 4));
	m_znmcu->trackball_read(BIT(data, 5));
	m_znmcu->select((data & 0x8c) != 0x8c); // TODO
...
public:
	...
	void coh1000c(machine_config &config) ATTR_COLD
	{
		zn1_1mb_vram(config);
		capcom_zn(config);
	}

	void coh1002c(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		capcom_zn(config);
	}

	void coh3002c(machine_config &config) ATTR_COLD
	{
		zn2(config);
		capcom_zn(config);
	}
...
protected:
	void capcom_zn(machine_config &config) ATTR_COLD
	{
		cat702<0>(config);
		cat702<1>(config);
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1f000000, 0x1f3fffff).rom().region(m_bankedroms, 0);
		map(0x1f400000, 0x1f7fffff).bankr(m_rombank);
		map(0x1fb00000, 0x1fb00000).w(FUNC(capcom_zn_state::bank_w));
		map(0x1fb40010, 0x1fb40013).portr("KICK1");
		map(0x1fb40020, 0x1fb40023).portr("KICK2");
...
protected:
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1f000000, 0x1f7fffff).bankr(m_rombank);
		map(0x1fb40000, 0x1fb40000).w(FUNC(taito_fx_state::bank_w));
	}
	...
	void bank_w(uint8_t data)
	{
		...
		m_rombank->set_entry(BIT(data, 0, 2));
	}
...
public:
	...
	void coh1000ta(machine_config &config) ATTR_COLD
	{
		zn1_1mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
protected:
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1fb80000, 0x1fb80000).w(m_tc0140syt, FUNC(tc0140syt_device::master_port_w));
		map(0x1fb80002, 0x1fb80002).rw(m_tc0140syt, FUNC(tc0140syt_device::master_comm_r), FUNC(tc0140syt_device::master_comm_w));
	}
	...
	void sound_map(address_map &map) ATTR_COLD
	{
		...
		map(0xe000, 0xe003).rw(m_ym2610b, FUNC(ym2610b_device::read), FUNC(ym2610b_device::write));
	...
	required_device<z80_device> m_audiocpu;
	required_device<ym2610b_device> m_ym2610b;
...
public:
	...
	void coh1000tb(machine_config &config) ATTR_COLD
	{
		zn1_1mb_vram(config);
		taito_fx1b(config);
	}

	void coh1002tb(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		taito_fx1b(config);
	}
...
protected:
	void taito_fx1b(machine_config &config) ATTR_COLD
	{
		cat702<0>(config);
		cat702<1>(config);
...
public:
	...
	void coh1000w(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
		...
		VT83C461(config, m_vt83c461).options(ata_devices, "hdd", nullptr, false);
		m_vt83c461->irq_handler().set(FUNC(primrag2_state::vt83c461_irq)).invert();
	}
...
protected:
	virtual void driver_start() override ATTR_COLD
	{
		...
		save_item(NAME(m_vt83c461_irqctrl));
	}
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1f000000, 0x1f000003).w(m_watchdog, FUNC(watchdog_timer_device::reset16_w)).umask16(0xffff); // ds1232s
		map(0x1f7e4000, 0x1f7e4fff).rw(FUNC(primrag2_state::vt83c461_16_r), FUNC(primrag2_state::vt83c461_16_w));
		map(0x1f7e8000, 0x1f7e8003).rw(FUNC(primrag2_state::vt83c461_irqctrl_r), FUNC(primrag2_state::vt83c461_irqctrl_w));
	...
	void dma_read(uint32_t *ram, uint32_t address, int32_t size)
	{
		LOGDMA("%s dma_read(0x%08x, 0x%08x)\n", machine().describe_context(), address, size << 2);

		size <<= 1;
		while (size > 0)
		{
			psxwriteword(ram, address, m_vt83c461->read_cs0(0, 0xffff));
			address += 2;
			size--;
		}
	}

	void dma_write(uint32_t *ram, uint32_t address, int32_t size)
	{
		LOGDMA("%s dma_write(0x%08x, 0x%08x)\n", address, size << 2);
	}
	...
	uint16_t vt83c461_16_r(offs_t offset, uint16_t mem_mask = ~0)
	{
		...
		if (offset >= 0x30 / 2 && offset < 0x40 / 2)
			data = m_vt83c461->config_r((offset / 2) & 3) >> shift;
		else if (offset >= 0x1f0 / 2 && offset < 0x1f8 / 2)
			data = m_vt83c461->read_cs0((offset / 2) & 1, mem_mask << shift) >> shift;
		else if (offset >= 0x3f0 / 2 && offset < 0x3f8 / 2)
			data = m_vt83c461->read_cs1((offset / 2) & 1, mem_mask << shift) >> shift;
		else if (!machine().side_effects_disabled())
			LOG("%s vt83c461_16_r(0x%04x, 0x%04x) 0x%04x\n", machine().describe_context(), offset, mem_mask, 0xffff);
	...
	void vt83c461_16_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0)
	{
		...
		if (offset >= 0x30 / 2 && offset < 0x40 / 2)
			...
			LOG("%s vt83c461_16_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
	}

	uint16_t vt83c461_irqctrl_r(offs_t offset, uint16_t mem_mask = ~0)
	{
		LOGDMA("%s vt83c461_irqctrl_r %08x %04x %04x\n", machine().describe_context(), offset, m_vt83c461_irqctrl, mem_mask);
		return m_vt83c461_irqctrl;
	}

	void vt83c461_irqctrl_w(offs_t offset, uint16_t data, uint16_t mem_mask)
	{
		LOGDMA("%s vt83c461_irqctrl_w %08x %04x %04x\n", machine().describe_context(), offset, data, mem_mask);

		COMBINE_DATA(&m_vt83c461_irqctrl);
	}
	...
	uint16_t vt83c461_32_r(offs_t offset, uint16_t mem_mask = ~0)
	{
		...
		if (offset == 0x1f0 / 2)
		{
			uint32_t data32 = m_vt83c461->read_cs0(0);
			m_vt83c461_latch = data32 >> 16;
			data = data32 & 0xffff;
		}
		else if (offset == 0x1f2 / 2)
			data = m_vt83c461_latch;
		else if (!machine().side_effects_disabled())
			LOG("%s vt83c461_32_r(0x%04x, 0x%04x) 0x%04x\n", machine().describe_context(), offset, mem_mask, 0xffff);
	...
	{
		LOG("%s vt83c461_32_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
	}

	inline void psxwriteword(uint32_t *ram, uint32_t address, uint16_t data)
	{
		*((uint16_t *)((uint8_t *)ram + WORD_XOR_LE(address))) = data;
	}

	void vt83c461_irq(int state)
	{
		m_irq->intin10(!(state || !BIT(m_vt83c461_irqctrl, 0)));
	}

	required_device<vt83c461_device> m_vt83c461;
	required_device<watchdog_timer_device> m_watchdog;

	uint16_t m_vt83c461_irqctrl;
...
public:
	...
	void coh1002e(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
protected:
	...
	virtual void znsecsel_w(uint8_t data) override
	{
		zn_state::znsecsel_w(data);
...
public:
	...
	static constexpr bool use_blprot = false;

	void beastrzrb(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);

		if (use_blprot)
		{
			// TODO: fix starting a game
			auto &sio0(*m_maincpu->subdevice<psxsio0_device>("sio0"));
			sio0.sck_handler().append(FUNC(beastrzrb_state::blprot_sck));
			sio0.txd_handler().append(FUNC(beastrzrb_state::blprot_rxd));
		}
		else
			cat702<0>(config);

		// TODO: figure out where cat702_2 values come from
		cat702<1>(config);
...
private:
	virtual void driver_start() override ATTR_COLD
	{
		...
		m_blprot->base()[0x1674] = 0x00; // skip red screen
		m_blprot->base()[0x20f0] = 0x1f; // skip error F930

		save_item(NAME(m_blprot_select));
		save_item(NAME(m_blprot_rxd));
		save_item(NAME(m_blprot_sck));
		save_item(NAME(m_blprot_index));
	...
	void oki_map(address_map &map) ATTR_COLD
	{
		...
		map(0x030000, 0x03ffff).bankr(m_okibank);
	}

	virtual void znsecsel_w(uint8_t data) override
	{
		blprot_select(BIT(data, 2));

		zn_state::znsecsel_w(data);
	}

	void blprot_select(int state)
	{
		if (!m_blprot_select && state)
			cat702_dataout<0>(1);

		m_blprot_select = state;
	}

	void blprot_rxd(int state)
	{
		m_blprot_rxd = state;
	}

	void blprot_sck(int state)
	{
		if (!m_blprot_sck && state && !m_blprot_select)
		{
			if (!(m_blprot_index & 7))
				LOGSEC("blprot[%d] %02x\n", (m_blprot_index >> 3) << 1, m_blprot->base()[(m_blprot_index >> 3) << 1]);

			cat702_dataout<0>(BIT(m_blprot->base()[(m_blprot_index >> 3) << 1], m_blprot_index & 7));
			LOGSEC("blprot %d %d (%d %d %d)\n", m_blprot_index, BIT(m_blprot->base()[(m_blprot_index >> 3) << 1], m_blprot_index & 7), m_cat702_dataout[0], m_cat702_dataout[1], m_znmcu_dataout);
			m_blprot_index++;
		}
	...
	required_memory_region m_blprot;
	required_memory_bank m_okibank;
	uint8_t m_blprot_select;
	uint8_t m_blprot_rxd;
	uint8_t m_blprot_sck;
	uint32_t m_blprot_index;
};
...
public:
	...
	void bam2(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
	...
	void bam2cdrom(machine_config &config) ATTR_COLD
	{
		bam2(config);

		m_ata->slot(0).set_default_option("cdrom");
	}

	void bam2hdd(machine_config &config) ATTR_COLD
	{
		bam2(config);

		// Quantum Fireball CR
		m_ata->slot(0).set_default_option("hdd");
	}
...
protected:
	...
	virtual void coin_w(uint8_t data) override
	{
		...
		machine().bookkeeping().coin_counter_w(0, BIT(m_coin, 0));
		machine().bookkeeping().coin_counter_w(2, BIT(m_coin, 2));
		machine().bookkeeping().coin_counter_w(1, BIT(m_coin, 4));
		machine().bookkeeping().coin_counter_w(3, BIT(m_coin, 6));
	}
	...
	uint16_t mcu_r(offs_t offset, uint16_t mem_mask = ~0)
	{
		...
		case 0:
			if (!machine().side_effects_disabled())
				LOGMCU("%s BAM2 MCU port 0 read (0x%04x) 0x%04x\n", machine().describe_context(), mem_mask, data);
			break;
		...
		case 2:
			...
			if (!machine().side_effects_disabled())
				LOGMCU("%s BAM2 MCU status read (0x%04x) 0x%04x\n", machine().describe_context(), mem_mask, data);
			break;

		default:
			if (!machine().side_effects_disabled())
				LOG("%s mcu_r(0x%04x, 0x%04x) 0x%04x\n", machine().describe_context(), offset, mem_mask, data);
			break;
		}
	...
	void mcu_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0)
	{
		switch (offset)
		{
		case 0:
		...
		case 1:
			LOGMCU("%s BAM2 MCU command (0x%04x, 0x%04x)\n", machine().describe_context(), data, mem_mask);
			m_mcu_command = data;
			break;

		default:
			LOG("%s mcu_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
			break;
		}
	}
...
public:
	...
	void coh1000a(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
protected:
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1fbfff00, 0x1fbfff03).w(FUNC(acclaim_zn_state::bank_w));
		map(0x1fbfff12, 0x1fbfff13).w(FUNC(acclaim_zn_state::acpsx_10_w));
	}

	virtual void bank_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0)
	{
		LOG("%s bank_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
	}

	void acpsx_10_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0)
	{
		LOG("%s acpsx_10_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
	}
...
public:
	...
	void jdredd(machine_config &config) ATTR_COLD
	{
		coh1000a(config);

		m_screen->screen_vblank().set(FUNC(jdredd_state::vblank));

		ATA_INTERFACE(config, m_ata).options(ata_devices, "hdd", nullptr, false);
		m_ata->irq_handler().set(m_irq, FUNC(psxirq_device::intin10));
	}
...
protected:
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		acclaim_zn_state::maincpu_program_map(map);

		map(0x1f000000, 0x1f1fffff).rom().region("bankedroms", 0);
		map(0x1fbfff80, 0x1fbfff8f).rw(m_ata, FUNC(ata_interface_device::cs1_r), FUNC(ata_interface_device::cs1_w));
		map(0x1fbfff90, 0x1fbfff9f).rw(m_ata, FUNC(ata_interface_device::cs0_r), FUNC(ata_interface_device::cs0_w));
	}
	...
	void vblank(int state)
	{
		if (state)
		{
			...
			int x = m_gun_x[m_gun_mux]->read();
			int y = m_gun_y[m_gun_mux]->read();

			if (x > 0x393 && x < 0xcb2 && y > 0x02d && y < 0x217)
...
public:
	nbajamex_state(const machine_config &mconfig, device_type type, const char *tag) :
		...
		m_71256(*this, "71256"),
		m_rax(*this, "rax"),
		m_bankmap(*this, "bankmap"),
		m_bankedroms(*this, "bankedroms"),
		m_rombank(*this, "rombank%u", 0)
	{
	}

	void nbajamex(machine_config &config) ATTR_COLD
	{
		coh1000a(config);

		NVRAM(config, m_71256, nvram_device::DEFAULT_ALL_1);
...
protected:
	virtual void driver_start() override ATTR_COLD
	{
		...
		m_71256->set_base(m_sram.get(), 0x8000);
	...
	{
		map(0x000000, 0x1fffff).mirror(0x800000).bankr(m_rombank[0]);
		map(0x200000, 0x207fff).mirror(0x800000).w(FUNC(nbajamex_state::backup_w));
		map(0x200000, 0x207fff).r(FUNC(nbajamex_state::backup_r));
		map(0xa00000, 0xffffff).bankr(m_rombank[1]);
	}

	uint8_t backup_r(offs_t offset)
	{
	...
	virtual void bank_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0) override
	{
		LOGBANK("%s bank_w(0x%04x, 0x%04x, 0x%04x)\n", machine().describe_context(), offset, data, mem_mask);
	...
	uint16_t sound_08_r(offs_t offset, uint16_t mem_mask = ~0)
	{
		// Sound related
		if (!machine().side_effects_disabled())
			LOG("%s sound_08_r(0x%04x, 0x%04x) %04x\n", machine().describe_context(), offset, mem_mask, 0x400);
		return 0x400;
	}
	...
	{
		if (!machine().side_effects_disabled())
			LOG("%s sound_80_r(0x%04x, 0x%04x) %04x\n", machine().describe_context(), offset, mem_mask, 0xffff);
		return 0xffff;
	}
	...
	required_device<nvram_device> m_71256;
...
public:
	...
	void coh1001l(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
protected:
	...
	void sound_map(address_map &map) ATTR_COLD
	{
		...
		map(0x200000, 0x200003).rw(m_ymz280b, FUNC(ymz280b_device::read), FUNC(ymz280b_device::write)).umask16(0x00ff);
	...
	{
		// irq ack maybe?
		LOG("%s sound_unk_w(0x%04x, 0x%04x, 0x%04x)\n", offset, data, mem_mask);
	}
...
public:
	...
	void coh1002v(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
protected:
	...
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		...
		map(0x1fb00000, 0x1fbfffff).bankr(m_rombank);
...
public:
	...
	void coh1002m(machine_config &config) ATTR_COLD
	{
		zn1_2mb_vram(config);
		cat702<0>(config);
		cat702<1>(config);
...
public:
	cbaj_state(const machine_config &mconfig, device_type type, const char *tag) :
		...
		m_audiocpu(*this, "audiocpu"),
		m_ymz280b(*this, "ymz280b"),
		m_fifo(*this, "lh540202_%u", 1)
	{
	}

	void cbaj(machine_config &config) ATTR_COLD
	{
		coh1002m(config);
...
protected:
	virtual void maincpu_program_map(address_map &map) override ATTR_COLD
	{
		tecmo_zn_state::maincpu_program_map(map);
	...
	{
		map.global_mask(0xff);
		map(0x84, 0x85).rw(m_ymz280b, FUNC(ymz280b_device::read), FUNC(ymz280b_device::write));
...
class glpracr2l_state :
	public tecmo_zn_state
{
public:
	glpracr2l_state(const machine_config &mconfig, device_type type, const char *tag) :
		tecmo_zn_state(mconfig, type, tag),
		m_linkcpu(*this, "linkcpu")
	{
	}

	void coh1002ml(machine_config &config) ATTR_COLD
	{
		coh1002m(config);
...
protected:
	...
	required_device<z80_device> m_linkcpu;
};
...
static INPUT_PORTS_START(bam2)
	...
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_NAME("%p Pedal") // P1 DOWN
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) PORT_16WAY // P1 DOWN
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_JOYSTICK_LEFT) PORT_16WAY
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_JOYSTICK_RIGHT) PORT_16WAY
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_NAME("%p Rhythm")
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_START) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_NAME("%p Start/Jammar") // P1 BUTTON2
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) PORT_NAME("%p Jammar") // P1 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_JOYSTICK_UP) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_PLAYER(3) PORT_NAME("Up Select") // P1 BUTTON3
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_UNUSED) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) // P1 BUTTON3

	PORT_MODIFY("P2")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_JOYSTICK_UP) PORT_16WAY PORT_PLAYER(2)
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_PLAYER(2) PORT_NAME("%p Pedal") // P2 DOWN
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) PORT_16WAY PORT_PLAYER(2) // P2 DOWN
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_JOYSTICK_LEFT) PORT_16WAY PORT_PLAYER(2)
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_JOYSTICK_RIGHT) PORT_16WAY PORT_PLAYER(2)
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_PLAYER(2) PORT_NAME("%p Rhythm")
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_START) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_PLAYER(2) PORT_NAME("%p Start/Jammar") // P1 BUTTON2
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) PORT_PLAYER(2) PORT_NAME("%p Jammar") PORT_PLAYER(2) // P1 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_PLAYER(3) PORT_NAME("Down Select") // P2 BUTTON3
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_UNUSED) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) // P2 BUTTON3

	PORT_MODIFY("SERVICE")
	PORT_SERVICE(0x01, IP_ACTIVE_LOW) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00)
	PORT_SERVICE_NO_TOGGLE(0x01, IP_ACTIVE_LOW) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01)

	PORT_MODIFY("SYSTEM")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_CONDITION("DSW", 0x01, EQUALS, 0x00) PORT_PLAYER(3) PORT_NAME("Enter") // START1
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_START1) PORT_CONDITION("DSW", 0x01, EQUALS, 0x01) // START1
...
static INPUT_PORTS_START(bldyror2)
	...
	PORT_MODIFY("P3")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON4) // P3 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_PLAYER(2) // P3 BUTTON2
INPUT_PORTS_END

static INPUT_PORTS_START(capcom6b)
	PORT_INCLUDE(zn2p)

	PORT_MODIFY("P3")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON4) // P3 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON5) // P3 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON6) // P3 BUTTON3

	PORT_MODIFY("P4")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_PLAYER(2) // P4 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_PLAYER(2) // P4 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_PLAYER(2) // P4 BUTTON3

	PORT_START("KICK1")
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_NAME("%p Kick Harness Button 4")
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_NAME("%p Kick Harness Button 5")
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_NAME("%p Kick Harness Button 6")
	PORT_BIT(0xfffffff8, IP_ACTIVE_LOW, IPT_UNUSED)

	PORT_START("KICK2")
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_NAME("%p Kick Harness Button 4") PORT_PLAYER(2)
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_NAME("%p Kick Harness Button 5") PORT_PLAYER(2)
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_NAME("%p Kick Harness Button 6") PORT_PLAYER(2)
	PORT_BIT(0xfffffff8, IP_ACTIVE_LOW, IPT_UNUSED)
INPUT_PORTS_END

static INPUT_PORTS_START(capcom4b)
	PORT_INCLUDE(capcom6b)
	...
	PORT_MODIFY("P3")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON3) // P3 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON4) // P3 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_UNUSED) // P3 BUTTON3

	PORT_MODIFY("P4")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_PLAYER(2) // P4 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_PLAYER(2) // P4 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_UNUSED) // P4 BUTTON3

	PORT_MODIFY("KICK1")
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_NAME("%p Kick Harness Button 3") // KICK1 BUTTON4
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_NAME("%p Kick Harness Button 4") // KICK1 BUTTON5
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_UNUSED) // KICK1 BUTTON6

	PORT_MODIFY("KICK2")
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON3) PORT_NAME("%p Kick Harness Button 3") PORT_PLAYER(2) // KICK2 BUTTON4
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_NAME("%p Kick Harness Button 4") PORT_PLAYER(2) // KICK2 BUTTON5
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_UNUSED) // KICK2 BUTTON6
INPUT_PORTS_END

static INPUT_PORTS_START(glpracr)
	PORT_INCLUDE(capcom4b)

	PORT_MODIFY("SYSTEM")
	PORT_BIT(0x22, IP_ACTIVE_LOW, IPT_UNUSED) // START2/COIN2

	PORT_MODIFY("P2")
	PORT_BIT(0x3f, IP_ACTIVE_LOW, IPT_UNUSED) // P2 UP/P2 DOWN/P2 LEFT/P2 RIGHT/P2 BUTTON1/P2 BUTTON2

	PORT_MODIFY("P3")
	PORT_BIT(0x30, IP_ACTIVE_LOW, IPT_UNUSED) // P3 BUTTON1/P3 BUTTON2

	PORT_MODIFY("P4")
	PORT_BIT(0x30, IP_ACTIVE_LOW, IPT_UNUSED) // P4 BUTTON1/P3 BUTTON2

	PORT_MODIFY("KICK1")
	PORT_BIT(0x0003, IP_ACTIVE_LOW, IPT_UNUSED) // KICK1 BUTTON4/KICK1 BUTTON5

	PORT_MODIFY("KICK2")
	PORT_BIT(0x0003, IP_ACTIVE_LOW, IPT_UNUSED) // KICK2 BUTTON4/KICK2 BUTTON5
INPUT_PORTS_END

static INPUT_PORTS_START(glpracr2)
	PORT_INCLUDE(zn2p)

	PORT_MODIFY("SYSTEM")
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_UNUSED) // COIN2
...
static INPUT_PORTS_START(jdredd)
	...
	PORT_MODIFY("P1")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON2) // P1 BUTTON1
	PORT_BIT(0xef, IP_ACTIVE_LOW, IPT_UNUSED) // P1 UP/P1 DOWN/P1 LEFT/P1 RIGHT/P1 BUTTON2/P1 BUTTON3/P1 BUTTON4

	PORT_MODIFY("P2")
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON2) PORT_PLAYER(2) // P2 BUTTON1
	PORT_BIT(0xef, IP_ACTIVE_LOW, IPT_UNUSED) // P2 UP/P2 DOWN/P2 LEFT/P2 RIGHT/P2 BUTTON2/P2 BUTTON3/P2 BUTTON4

	PORT_MODIFY("SERVICE")
	PORT_BIT(0x08, IP_ACTIVE_LOW, IPT_CUSTOM) PORT_READ_LINE_MEMBER(FUNC(jdredd_state::gun_mux_r))
	PORT_BIT(0x30, IP_ACTIVE_LOW, IPT_UNUSED) // P1 BUTTON5/P2 BUTTON5
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON1) // P3 BUTTON5
	PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_BUTTON1) PORT_PLAYER(2) // P4 BUTTON5

	PORT_MODIFY("SYSTEM")
	PORT_BIT(0x0c, IP_ACTIVE_LOW, IPT_UNUSED) // START3/START4
	...
	PORT_START("GUN1X")
	PORT_BIT(0xffff, 0x0822, IPT_LIGHTGUN_X) PORT_CROSSHAIR(X, 1.0, 0.0, 0) PORT_MINMAX(0x393 - 1, 0xcb2 + 1) PORT_SENSITIVITY(100) PORT_KEYDELTA(50)

	PORT_START("GUN1Y")
	PORT_BIT(0xffff, 0x0122, IPT_LIGHTGUN_Y) PORT_CROSSHAIR(Y, 1.0, 0.0, 0) PORT_MINMAX(0x02d - 1, 0x217 + 1) PORT_SENSITIVITY(100) PORT_KEYDELTA(15)
...
static INPUT_PORTS_START(plsmaswd)
	PORT_INCLUDE(capcom6b)

	PORT_MODIFY("P3")
	PORT_BIT(0x60, IP_ACTIVE_LOW, IPT_UNUSED) // P3 BUTTON2/P3 BUTTON3

	PORT_MODIFY("P4")
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_UNUSED) // P4 BUTTON2/P4 BUTTON3

	PORT_MODIFY("KICK1")
	PORT_BIT(0x0006, IP_ACTIVE_LOW, IPT_UNUSED) // KICK1 BUTTON5/KICK1 BUTTON6

	PORT_MODIFY("KICK2")
	PORT_BIT(0x0006, IP_ACTIVE_LOW, IPT_UNUSED) // KICK2 BUTTON5/KICK2 BUTTON6
INPUT_PORTS_END
...
static INPUT_PORTS_START(primrag2)
	...
	PORT_MODIFY("P4")
	PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_BUTTON4) // P4 UP
	PORT_BIT(0x02, IP_ACTIVE_LOW, IPT_BUTTON5) // P4 DOWN
	PORT_BIT(0x04, IP_ACTIVE_LOW, IPT_BUTTON6) // P4 LEFT
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_PLAYER(2) // P4 BUTTON1
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_PLAYER(2) // P4 BUTTON2
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_PLAYER(2) // P4 BUTTON3
	PORT_BIT(0x88, IP_ACTIVE_LOW, IPT_UNUSED) // P4 RIGHT/P4 BUTTON4
INPUT_PORTS_END

static INPUT_PORTS_START(raystorm)
	PORT_INCLUDE(znt2p)
...
static INPUT_PORTS_START(strider2)
	PORT_INCLUDE(capcom6b)
	...
	PORT_MODIFY("P3")
	PORT_BIT(0x70, IP_ACTIVE_LOW, IPT_UNUSED) // P3 BUTTON1/P3 BUTTON2/P3 BUTTON3

	PORT_MODIFY("P4")
	PORT_BIT(0x70, IP_ACTIVE_LOW, IPT_UNUSED) // P4 BUTTON1/P4 BUTTON2/P4 BUTTON3

	PORT_MODIFY("KICK1")
	PORT_BIT(0x0007, IP_ACTIVE_LOW, IPT_UNUSED) // KICK1 BUTTON4/KICK1 BUTTON5/KICK1 BUTTON6

	PORT_MODIFY("KICK2")
	PORT_BIT(0x0007, IP_ACTIVE_LOW, IPT_UNUSED) // KICK2 BUTTON4/KICK2 BUTTON5/KICK1 BUTTON6
INPUT_PORTS_END

static INPUT_PORTS_START(tgmj)
	PORT_INCLUDE(capcom6b)
	...
	PORT_MODIFY("P4")
	PORT_BIT(0x70, IP_ACTIVE_LOW, IPT_UNUSED) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x00)
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_PLAYER(2)
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_PLAYER(2)
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_PLAYER(2)

	PORT_MODIFY("KICK1")
	PORT_BIT(0x0007, IP_ACTIVE_LOW, IPT_UNUSED) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x00)
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 4")
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 5")
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 6")

	PORT_MODIFY("KICK2")
	PORT_BIT(0x0007, IP_ACTIVE_LOW, IPT_UNUSED) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x00)
	PORT_BIT(0x0001, IP_ACTIVE_LOW, IPT_BUTTON4) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 4") PORT_PLAYER(2)
	PORT_BIT(0x0002, IP_ACTIVE_LOW, IPT_BUTTON5) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 5") PORT_PLAYER(2)
	PORT_BIT(0x0004, IP_ACTIVE_LOW, IPT_BUTTON6) PORT_CONDITION("DEBUG", 0x01, EQUALS, 0x01) PORT_NAME("%p Kick Harness Button 6") PORT_PLAYER(2)
INPUT_PORTS_END
...
#define CAPCOM_ZN1_BIOS \
	...
	ROM_REGION( 0x002000, "upd78081", 0 ) \
...
ROM_START( coh1000c )
	...
	ROM_REGION( 0x400000, "qsound", ROMREGION_ERASEFF )
...
ROM_START( ts2 )
	...
	ROM_LOAD( "ts2_02.2e", 0x000000, 0x020000, CRC(2f45c461) SHA1(513b6b9b5a2f7c567c30c958e0e13834cd9bd266) )
	ROM_FILL(              0x020000, 0x020000, 0xff ) // 3e

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "ts2-01m.3b", 0x000000, 0x400000, CRC(d7a505e0) SHA1(f1b0cdea712101f695bd326eccd753eb79a07490) ) // labeled TS2-01M  KM23C320000-15
...
ROM_START( starglad )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "ps1_02a.2e", 0x000000, 0x020000, CRC(b854df92) SHA1(ea71a613b5b19ec7e9c6e342e7743d320582a6bb) )
	ROM_LOAD( "ps1_03a.3e", 0x020000, 0x020000, CRC(a2562fbb) SHA1(3de02a4aa7ea620961ca2a5c331f38134033db79) )
...
ROM_START( sfex )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "sfe_02.2e",  0x000000, 0x020000, CRC(1908475c) SHA1(99f68cff2d92f5697eec0846201f6fb317d5dc08) )
	ROM_LOAD( "sfe_03.3e",  0x020000, 0x020000, CRC(95c1e2e0) SHA1(383bbe9613798a3ac6944d18768280a840994e40) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "sfe-01m.3b", 0x000000, 0x400000, CRC(f5afff0d) SHA1(7f9ac32ba0a3d9c6fef367e36a92d47c9ac1feb3) )
...
ROM_START( rvschool )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "jst_02.2e",  0x000000, 0x020000, CRC(7809e2c3) SHA1(0216a665f7978bc8db3f7fdab038e1c7aa120844) )
	ROM_LOAD( "jst_03.3e",  0x020000, 0x020000, CRC(860ff24d) SHA1(eea72fa5eaf407a112a5b3daf60f7ac8ad191cc7) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "jst-01m.3b", 0x000000, 0x400000, CRC(9a7c98f9) SHA1(764c6c4f41047e1f36d2dceac4aa9b943a9d529a) )
...
ROM_START( sfex2 )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "ex2_02.2e",  0x000000, 0x020000, CRC(9489875e) SHA1(1fc9985ff98232c63ea8d05a69f7d77cdf72919f) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "ex2-01m.3a", 0x000000, 0x400000, CRC(14a5bb0e) SHA1(dfe3c3a53bd4c58743d8039b5344d3afbe2a9c24) )
...
ROM_START( plsmaswd )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "sg2_02.2e",  0x000000, 0x020000, CRC(415ee138) SHA1(626083c8705f012552691c450f95401ddc88065b) )
	ROM_LOAD( "sg2_03.3e",  0x020000, 0x020000, CRC(43806735) SHA1(88d389bcc79cbd4fa1f4b62008e171a897e77652) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "sg2-01m.3a", 0x000000, 0x400000, CRC(643ea27b) SHA1(40747432d5cfebac54d3824b6a6f26b5e7742fc1) )
...
ROM_START( tgmj )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "ate_02.2e",  0x000000, 0x020000, CRC(f4f6e82f) SHA1(ad6c49197a60f456367c9f78353741fb847819a1) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "ate-01m.3a", 0x000000, 0x400000, CRC(a21c6521) SHA1(560e4855f6e00def5277bdd12064b49e55c3b46b) )
...
ROM_START( techromn )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "kio_02.2e",  0x000000, 0x020000, CRC(174309b3) SHA1(b35b9c3905d2fabaa8410f70f7b382e916c89733) )
	ROM_LOAD( "kio_03.3e",  0x020000, 0x020000, CRC(0b313ae5) SHA1(0ea39305ca30f376930e39b134fd1a52200624fa) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "kio-01m.3a", 0x000000, 0x400000, CRC(6dc5bd07) SHA1(e1755a48465f741691ea0fa1166cb2dc09210ed9) )
...
ROM_START( sfex2p )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "x2p_02.2e",  0x000000, 0x020000, CRC(3705de5e) SHA1(847007ca271da64bf13ffbf496d4291429eee27a) )
	ROM_LOAD( "x2p_03.3e",  0x020000, 0x020000, CRC(6ae828f6) SHA1(41c54165e87b846a845da581f408b96979288158) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "x2p-01m.3a", 0x000000, 0x400000, CRC(14a5bb0e) SHA1(dfe3c3a53bd4c58743d8039b5344d3afbe2a9c24) )
...
ROM_START( strider2 )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "hr2_02.2e",  0x000000, 0x020000, CRC(acd8d385) SHA1(5edb61c3d66d2d09a28a71db52eee3a9f7db8c9d) )

	ROM_REGION( 0x400000, "qsound", 0 )
	ROM_LOAD16_WORD_SWAP( "hr2-01m.3a", 0x000000, 0x200000, CRC(510a16d1) SHA1(05f10c2921a4d3b1fab4d0a4ea06351809bdbb07) )
...
ROM_START( glpracr2l )
	...
	ROM_REGION( 0x040000, "linkcpu", 0 )
	ROM_LOAD( "link3118.bin", 0x000000, 0x040000, CRC(a4d4761e) SHA1(3fb25dfa5220d25093588d9501e0666214491100) )
...
ROM_START( cbaj )
	...
	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "cbaj_z80.3118", 0x000000, 0x040000, CRC(92b02ad2) SHA1(f72317679ecbd8a0c3b081baaf9ff20a8c9ec00f) )

	ROM_REGION( 0x800000, "ymz280b", 0 )
	...
	ROM_REGION( 0x000c00, "misc", 0 )
	ROM_LOAD( "gal16v8d-15lp.sop-rom1.uo117",   0x000000, 0x000117, CRC(cf8ebc23) SHA1(0662f8ba418eb9187fb7a86cc8c0d86220dcdbf0) )
	ROM_LOAD( "gal16v8d-15lp.sop-rom2b.uo4138", 0x000200, 0x000117, CRC(098662c1) SHA1(5fcf66b16f840129aa61e9cf2b2951c37a5cbabf) )
	ROM_LOAD( "gal16v8d-15lp.sop-rom3.uo3125",  0x000400, 0x000117, CRC(f9c92db7) SHA1(1c2818f0fceedca914a0cbb74c2237a8c3798906) )
	ROM_LOAD( "gal16v8d-15lp.sop-rom4c.uo4131", 0x000600, 0x000117, CRC(e87179ef) SHA1(9942ff71d39924d938aa5e7c3d0c1aa9a38c5e77) )
	ROM_LOAD( "gal16v8d-15lp.sop-rom5b.uo4128", 0x000800, 0x000117, CRC(f32a1803) SHA1(8251bad3fe5a0ebf6cba4c1a67aa09e391289c65) )
	ROM_LOAD( "gal16v8d-15lp.sop-rom6a.uo4134", 0x000a00, 0x000117, CRC(40e1f6f2) SHA1(e7703e6db5d4f23c34c633ec147a88d2b0f681d5) )
ROM_END
...
ROM_START( cbajbl )
	...
	ROM_LOAD16_BYTE( "uo120.bin",  0x0000000, 0x080000, CRC(89286229) SHA1(18a84ef648ec3b79707eb42b55563adf38dffd0d) )
	ROM_LOAD( "uo216.bin",         0x0400000, 0x200000, CRC(1b18e0e9) SHA1(306b4849f707c5b643992e3880a34b894fc9f0e1) ) // cb-00.216 [1/2]
	ROM_LOAD( "uo216h.bin",        0x0600000, 0x200000, CRC(bb86ee76) SHA1(9eead295d8fec3708c1e693d3d58346cc214580c) ) // cb-00.216 [2/2]
	ROM_LOAD( "uo217.bin",         0x0800000, 0x200000, CRC(02d8dfcd) SHA1(b83636374fcae6aa1c3a320fafcc7bd7890c0bc6) ) // cb-01.217 [1/2]
	ROM_LOAD( "uo217h.bin",        0x0a00000, 0x200000, CRC(6e0f34da) SHA1(e7384a3019dcc0f69335635056ae3fac89fb4b52) ) // cb-01.217 [2/2]
	ROM_LOAD( "uo218.bin",         0x0c00000, 0x200000, CRC(14eaaff6) SHA1(b38f541b9f0397edee071d883b2801fa40cba127) ) // cb-02.218 [1/2]
	ROM_LOAD( "uo218h.bin",        0x0e00000, 0x200000, CRC(9f6420dc) SHA1(3b354bdfc54ac5d69c229d897cabc807c21c61bc) ) // cb-02.218 [2/2]
	ROM_LOAD( "uo219.bin",         0x1000000, 0x200000, CRC(89e727b1) SHA1(09e69708c5b8244dbacd09dce6558cf917a58623) ) // cb-03.219 [1/2]
	ROM_LOAD( "uo219h.bin",        0x1200000, 0x200000, BAD_DUMP CRC(727b7ae5) SHA1(182b63b77965a25591bda71575af464f6e3b26f8) ) // cb-03.219 [2/2] 1 bit different
	...
	ROM_LOAD( "uo223h.bin",        0x2200000, 0x200000, CRC(eb5b6b3a) SHA1(ad595053b852a45b85f061dfa960fa04cf74eb9f) ) // cb-07.223 [2/2]
	ROM_LOAD( "uo323.bin",         0x2400000, 0x200000, CRC(e9140f6c) SHA1(a9e8c3b94de319a5fd5327111339c76c65371ba9) ) // cb-08.323 [1/2]

	ROM_REGION( 0x040000, "audiocpu", 0 )
	ROM_LOAD( "near_z80.bin",  0x000000,  0x020000, CRC(10b30c5b) SHA1(8c4af71dc14c43a2b32377737b6dabc393b3c141) ) // cbaj_z80.3118 [1/16] duplicated 16 times

	ROM_REGION( 0x800000, "ymz280b", 0 )
	ROM_LOAD( "uo5120.bin",   0x000000, 0x200000, CRC(973bc8fe) SHA1(ab37fdebff5daee469f51838a6f498541c1c9e07) ) // cb-vo.5120 [1/2]
	ROM_LOAD( "uo5120h.bin",  0x200000, 0x200000, CRC(01bede31) SHA1(ed7a2eaabe21127b6440cc20d3089a880ce428d5) ) // cb-vo.5120 [2/2]
	ROM_LOAD( "uo5121.bin",   0x400000, 0x200000, CRC(76112372) SHA1(f69860fbffd33aee6cdf475c091987467bca274a) ) // cb-se.5121 [1/2]
...
ROM_START( glpracr3 )
	...
	ROM_REGION( 0x000200, "misc", 0 )
	ROM_LOAD( "rom1.gal16v8d.u0117.bin", 0x000000, 0x000117, CRC(cf8ebc23) SHA1(0662f8ba418eb9187fb7a86cc8c0d86220dcdbf0) ) // unprotected, verified on PCB, near the MG08 security chip
...
ROM_START( 1on1gov )
	...
	ROM_LOAD( "at28c16", 0x000000, 0x000800, CRC(fe992f29) SHA1(73c9c4d40abd8f3a95d4eb20f3c65f3a5cdd1203) )
...
ROM_START( coh1000t )
	...
	ROM_REGION( 0x200000, "ym2610b:adpcma", ROMREGION_ERASEFF )
	ROM_REGION( 0x000008, "cat702_2", 0 ) ROM_COPY( "cat702_1", 0x000000, 0x000000, 0x000008 )
ROM_END
...
ROM_START( ftimpcta )
	...
	ROM_LOAD( "e25-10.14", 0x000000, 0x080000, CRC(2b2ad1b1) SHA1(6d064d0b6805d43ce42929ac8f5645b56384f53c) )

	ROM_REGION32_LE( 0x600000, "taito_zoom:zsg2", 0 )
	ROM_LOAD( "e25-04.27", 0x000000, 0x400000, CRC(09a66d35) SHA1(f0df24bc9bfc9eb0f5150dc035c19fc5b8a39bf9) )
	ROM_LOAD( "e25-05.28", 0x400000, 0x200000, CRC(3fb57636) SHA1(aa38bfac11ecf10fd55143cf4525a2a529be8bb6) )
...
ROM_START( gdarius )
	...
	ROM_REGION( 0x080000, "taito_zoom:mn10200", 0 )
	ROM_LOAD( "e39-07.14", 0x000000, 0x080000, CRC(2252c7c1) SHA1(92b9908e0d87cad6587f1acc0eef69eaae8c6a98) )

	ROM_REGION32_LE( 0x400000, "taito_zoom:zsg2", 0 )
	ROM_LOAD( "e39-04.27", 0x000000, 0x400000, CRC(6ee35e68) SHA1(fdfe63203d8cecf84cb869039fb893d5b63cdd67) )
...
ROM_START( mgcldate )
	...
	ROM_LOAD( "e32-07.22", 0x000000, 0x020000, CRC(adf3feb5) SHA1(bae5bc3fad99a92a3492be1b775dab861007eb3b) )

	ROM_REGION( 0x400000, "ym2610b:adpcma", 0 )
	ROM_LOAD( "e32-04.15", 0x000000, 0x400000, CRC(c72f9eea) SHA1(7ab8b412a8ed00a42016acb7d13d3b074155780a) )
...
ROM_START( mgcldtex )
	...
	ROM_REGION( 0x020000, "audiocpu", 0 )
	ROM_LOAD( "e32-10.22", 0x000000, 0x020000, CRC(adf3feb5) SHA1(bae5bc3fad99a92a3492be1b775dab861007eb3b) )
...
ROM_START( psyforce )
	...
	ROM_LOAD16_BYTE( "e22-06_.7", 0x0000000, 0x080000, CRC(f6341d63) SHA1(99dc27aa694ae5951148054291912a486726e8c9) ) // E22-06*
	...
	ROM_REGION( 0x020000, "audiocpu", 0 )
	ROM_LOAD( "e22-07.22", 0x000000, 0x020000, CRC(739af589) SHA1(dbb4d1c6d824a99ccf27168e2c21644e19811523) )

	ROM_REGION( 0x200000, "ym2610b:adpcma", 0 )
...
ROM_START( psyforcej )
	...
	ROM_LOAD16_BYTE( "e22-10_.7", 0x0000000, 0x080000, CRC(f6341d63) SHA1(99dc27aa694ae5951148054291912a486726e8c9) ) // E22-10*
	ROM_LOAD( "e22-02.16",        0x0800000, 0x200000, CRC(03b50064) SHA1(0259537e86b266b3f34308c4fc0bcc04c037da71) )
	ROM_LOAD( "e22-03.19",        0x0a00000, 0x200000, CRC(8372f839) SHA1(646b3919b6be63412c11850ec1524685abececc0) )
	ROM_LOAD( "e22-04.21",        0x0c00000, 0x200000, CRC(397b71aa) SHA1(48743c362503c1d2dbeb3c8be4cb2aaaae015b88) )
...
ROM_START( raystorm )
	...
	ROM_LOAD( "e24-09.14", 0x000000, 0x080000, CRC(808589e1) SHA1(46ada4c6d68c2462186a0b962abb435ee740c0ba) )

	ROM_REGION32_LE( 0x400000, "taito_zoom:zsg2", 0 )
	ROM_LOAD( "e24-04.27", 0x000000, 0x400000, CRC(f403493a) SHA1(3e49fd2a060a3893e26f14cc3cf47c4ba91e17d4) )
...
ROM_START( sfchamp )
	...
	ROM_REGION( 0x020000, "audiocpu", 0 )
	ROM_LOAD( "e18-09.22", 0x000000, 0x020000, CRC(bb5a5319) SHA1(0bb700cafc157d3af663cc9bebb8167487ff2852) )

	ROM_REGION( 0x200000, "ym2610b:adpcma", 0 )
	ROM_LOAD( "e18-01.15", 0x000000, 0x200000, CRC(dbd1408c) SHA1(ef81064f2f95e5ae25eb1f10d1e78f27f9e294f5) )
...
ROM_START( coh1002e )
	...
	ROM_REGION32_LE( 0x1800000, "bankedroms", ROMREGION_ERASEFF )
	ROM_REGION( 0x080000, "audiocpu", ROMREGION_ERASEFF )
	ROM_REGION( 0x400000, "ymf", ROMREGION_ERASEFF )
	ROM_REGION( 0x000008, "cat702_2", 0 ) ROM_COPY( "cat702_1", 0x000000, 0x000000, 0x000008 )
ROM_END
...
ROM_START( beastrzrb )
	...
	ROM_REGION( 0x001000, "audiocpu", 0 )
	...
	ROM_LOAD( "27c240.3", 0x000000, 0x080000, BAD_DUMP CRC(509cdc8b) SHA1(8b92b79be09de56e7d40c2d02fcbeca92bb60226) ) // replay attack, upper & lower bytes identical
...
ROM_START( brvblade )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_world", 0x000000, 0x000800, CRC(fe7f7d34) SHA1(18f6ae14e57afa668b3eef821b4cf0a7599a21ac) )
...
ROM_START( brvbladeu )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_usa", 0x000000, 0x000800, CRC(0a2c042f) SHA1(147651d2e55873a82295214b3b0bd6c46cdad239) )
...
ROM_START( brvbladej )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_japan", 0x000000, 0x000800, CRC(59e2d326) SHA1(53f48b6ad7243aa92b54863515f104a7a54b2810) )
...
ROM_START( brvbladea )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_asia", 0x000000, 0x000800, CRC(adb1aa3d) SHA1(118a17b8a15108666e4f9f2d1798031b3b893536) )
...
ROM_START( bldyror2 )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_world", 0x000000, 0x000800, CRC(01b42397) SHA1(853553a38e81e64a17c040173b29c7bfd6f79f31) )
...
ROM_START( bldyror2u )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_usa", 0x000000, 0x000800, CRC(b78d6fc3) SHA1(49d8b6f44c31d74f36cba981af7f4c7e23dd9007) )
...
ROM_START( bldyror2j )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_japan", 0x000000, 0x000800, CRC(6cb55630) SHA1(b840bc0339485dd82f7c9aa669faf90ae371218f) )
...
ROM_START( bldyror2a )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16_asia", 0x000000, 0x000800, CRC(da8c1a64) SHA1(14cbb751f498c96d9d8fce3eea3781ebc45f6291) )
...
ROM_START( bam2 )
	...
	ROM_REGION( 0x008000, "h83644", 0)
	ROM_LOAD( "hd64f3644.u2", 0x000000, 0x008000, NO_DUMP )
...
ROM_START( bam2a )
	...
	DISK_IMAGE( "bam2cd", 0, SHA1(cdedc8f9eb67bed267c899d679f986bf1748bd00) )
ROM_END
...
ROM_START( primrag2 )
	...
	ROM_REGION( 0x000800, "at28c16", 0 )
	ROM_LOAD( "at28c16", 0x000000, 0x000800, CRC(62c21283) SHA1(6013d831fb97c025682603a1bb9d62bd93e1b893) )
...
ROM_START( hvnsgate )
	...
	ROM_REGION( 0x800000, "ymz280b", 0 )
	ROM_FILL(                 0x000000, 0x400000, 0xff )
...
GAME( 1995, ts2j,      ts2,      coh1000c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Takara",         "Battle Arena Toshinden 2 (Japan 951124)",                  MACHINE_SUPPORTS_SAVE )
GAME( 1995, ts2ja,     ts2,      coh1000c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Takara",         "Battle Arena Toshinden 2 (Japan 951124, 32Mb mask ROMs)",  MACHINE_SUPPORTS_SAVE )
GAME( 1996, starglad,  coh1000c, coh1000c,  capcom4b, capcom_zn_state,  empty_init, ROT0, "Capcom",                  "Star Gladiator Episode I: Final Crusade (USA 960627)",     MACHINE_SUPPORTS_SAVE )
GAME( 1996, stargladj, starglad, coh1000c,  capcom4b, capcom_zn_state,  empty_init, ROT0, "Capcom",                  "Star Gladiator Episode I: Final Crusade (Japan 960627)",   MACHINE_SUPPORTS_SAVE )
GAME( 1996, glpracr,   coh1000c, coh1000c,  glpracr,  glpracr_state,    empty_init, ROT0, "Tecmo",                   "Gallop Racer (English Ver 10.17.K)",                       MACHINE_SUPPORTS_SAVE )
GAME( 1996, glpracrj,  glpracr,  coh1000c,  glpracr,  glpracr_state,    empty_init, ROT0, "Tecmo",                   "Gallop Racer (Japanese Ver 9.01.12)",                      MACHINE_SUPPORTS_SAVE )
GAME( 1996, sfex,      coh1000c, coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX (Europe 961219)",                        MACHINE_SUPPORTS_SAVE )
GAME( 1996, sfexu,     sfex,     coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX (USA 961219)",                           MACHINE_SUPPORTS_SAVE )
GAME( 1996, sfexa,     sfex,     coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX (Asia 961219)",                          MACHINE_SUPPORTS_SAVE )
GAME( 1996, sfexj,     sfex,     coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX (Japan 961130)",                         MACHINE_SUPPORTS_SAVE )
GAME( 1997, sfexp,     coh1000c, coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX Plus (USA 970407)",                      MACHINE_SUPPORTS_SAVE )
GAME( 1997, sfexpu1,   sfexp,    coh1002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX Plus (USA 970311)",                      MACHINE_SUPPORTS_SAVE )
...
GAME( 1998, sfex2u1,   sfex2,    coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 (USA 980312)",                          MACHINE_SUPPORTS_SAVE )
GAME( 1998, sfex2a,    sfex2,    coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 (Asia 980312)",                         MACHINE_SUPPORTS_SAVE )
GAME( 1998, sfex2h,    sfex2,    coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 (Hispanic 980312)",                     MACHINE_SUPPORTS_SAVE )
GAME( 1998, sfex2j,    sfex2,    coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 (Japan 980312)",                        MACHINE_SUPPORTS_SAVE )
GAME( 1998, plsmaswd,  coh3002c, coh3002c,  plsmaswd, capcom_zn_state,  empty_init, ROT0, "Capcom",                  "Plasma Sword: Nightmare of Bilstein (USA 980316)",         MACHINE_SUPPORTS_SAVE )
...
GAME( 1999, sfex2p,    coh3002c, coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 Plus (Europe 990611)",                  MACHINE_SUPPORTS_SAVE )
GAME( 1999, sfex2pu,   sfex2p,   coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 Plus (USA 990611)",                     MACHINE_SUPPORTS_SAVE )
GAME( 1999, sfex2pa,   sfex2p,   coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 Plus (Asia 990611)",                    MACHINE_SUPPORTS_SAVE )
GAME( 1999, sfex2ph,   sfex2p,   coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 Plus (Hispanic 990611)",                MACHINE_SUPPORTS_SAVE )
GAME( 1999, sfex2pj,   sfex2p,   coh3002c,  capcom6b, capcom_zn_state,  empty_init, ROT0, "Capcom / Arika",          "Street Fighter EX2 Plus (Japan 990611)",                   MACHINE_SUPPORTS_SAVE )
GAME( 1999, strider2,  coh3002c, coh3002c,  strider2, capcom_zn_state,  empty_init, ROT0, "Capcom",                  "Strider 2 (Europe 991213)",                                MACHINE_SUPPORTS_SAVE )
GAME( 1999, strider2u, strider2, coh3002c,  strider2, capcom_zn_state,  empty_init, ROT0, "Capcom",                  "Strider 2 (USA 991213)",                                   MACHINE_SUPPORTS_SAVE )
...
GAME( 1999, glpracr3j, glpracr3, coh1002m,  glpracr2, tecmo_zn_state,   empty_init, ROT0, "Tecmo",                   "Gallop Racer 3 (Japan)",                                   MACHINE_SUPPORTS_SAVE )
GAME( 1999, flamegun,  coh1002m, coh1002m,  zn2p,     tecmo_zn_state,   empty_init, ROT0, "Gaps Inc.",               "Flame Gunner (Export)",                                    MACHINE_SUPPORTS_SAVE )
GAME( 1999, flamegunj, flamegun, coh1002m,  zn2p,     tecmo_zn_state,   empty_init, ROT0, "Gaps Inc.",               "Flame Gunner (Japan)",                                     MACHINE_SUPPORTS_SAVE )
GAME( 1999, lpadv,     coh1002m, coh1002m,  zn2p,     tecmo_zn_state,   empty_init, ROT0, "Amuse World",             "Logic Pro Adventure (Japan)",                              MACHINE_SUPPORTS_SAVE )
GAME( 2000, tblkkuzu,  coh1002m, coh1002m,  zn2p,     tecmo_zn_state,   empty_init, ROT0, "Tamsoft / D3 Publisher",  "The Block Kuzushi (Japan)",                                MACHINE_SUPPORTS_SAVE )
GAME( 2000, 1on1gov,   coh1002m, coh1002m,  zn,       tecmo_zn_state,   empty_init, ROT0, "Tecmo",                   "1 on 1 Government (Japan)",                                MACHINE_SUPPORTS_SAVE )

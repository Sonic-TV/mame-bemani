...
uint32_t djmain_state::turntable_r(offs_t offset, uint32_t mem_mask)
{
	...
	if (ACCESSING_BITS_8_15)
	{
		const int turntable_switch = m_turntable_switch ? m_turntable_switch->read() : 0;
		const uint8_t pos = ((turntable_switch & (0x0f << (4 * m_turntable_select))) == 0 ? m_turntable_analog : m_turntable_digital)[m_turntable_select].read_safe(0);

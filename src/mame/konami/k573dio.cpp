...
uint16_t k573dio_device::decrypt_common(uint16_t data, uint16_t key)
{
...
uint16_t k573dio_device::decrypt_default(uint16_t data)
{
	const uint16_t derived_key = bitswap<16>(
		m_crypto_key1 ^ m_crypto_key2,
		15, 13, 14, 12,
		11, 10,  9,  7,
		8,  6,  5,  4,
		3,  1,  2,  0
	...
	if (BIT(m_crypto_key1, 14) ^ BIT(m_crypto_key1, 15))
...
{
	data = decrypt_common(data, m_crypto_key1);
	m_crypto_key1 = (m_crypto_key1 << 1) | (m_crypto_key1 >> 15);
	return data;
}
